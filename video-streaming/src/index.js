...
//
function sendViewedMessage(messageChannel, videoPath, params) {
    console.log(`Publishing message on "viewed" exchange.`);
        
    const msg = { videoPath: videoPath };
    const jsonMsg = JSON.stringify(msg);
    messageChannel.publish(`viewed-${params}`, "", Buffer.from(jsonMsg)); // Publish message to the "viewed" exchange.
}
...
function setupHandlers(app, messageChannel) {
    app.get("/video", (req, res) => { // Route for streaming video.
        ...
        if (req.query.id == 1){ sendViewedMessage(messageChannel, videoPath, 1); }
        else if (req.query.id == 2) { sendViewedMessage(messageChannel, videoPath, 2); }
        fs.stat(videoPath, (err, stats) => {
            ...
            sendViewedMessage(messageChannel, videoPath, 0); // Send message to "history" microservice that this video has been "viewed".
        });
    });
}
